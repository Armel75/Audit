...
export default function RecommendationFormModal({ isOpen, onClose, findingId, onSuccess }: RecommendationFormModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(data.error || 'Erreur lors de la création de la recommandation');
      }
        ...
        <div className="inline-block transform overflow-hidden rounded-lg bg-white text-left align-bottom shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-2xl sm:align-middle">
          <div className="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
            <div className="flex justify-between items-center mb-5">
              <h3 className="text-lg font-medium leading-6 text-slate-900">Nouvelle Recommandation</h3>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                <label htmlFor="actionPlan" className="block text-sm font-medium text-slate-700">Plan d'action *</label>
                <textarea
                  id="actionPlan"
                  required
                  rows={4}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label htmlFor="priorityId" className="block text-sm font-medium text-slate-700">Priorité</label>
                  <select
                    id="priorityId"
                    value={priorityId}
                ...
                <div>
                  <label htmlFor="departmentId" className="block text-sm font-medium text-slate-700">Département responsable</label>
                  <select
                    id="departmentId"
                    value={departmentId}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label htmlFor="assigneeName" className="block text-sm font-medium text-slate-700">Responsable (Nom)</label>
                  <input
                    type="text"
                    id="assigneeName"
                    value={assigneeName}
